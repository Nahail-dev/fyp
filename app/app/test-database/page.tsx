...
export default function TestDatabasePage() {
  ...
  const runTests = async () => {
    ...
    try {
      ...
      if (userError || !user) {
        updateResult('Get Current User', 'error', 'No authenticated user found');
        toast.error('You must be logged in to run tests');
        setLoading(false);
        return;
      }
      updateResult('Get Current User', 'success', `Logged in as: ${user.email}`);
      ...
      if (profileError) {
        updateResult('Fetch User Profile', 'error', `Error: ${profileError.message}`);
      } else {
        updateResult('Fetch User Profile', 'success', `Profile loaded: ${profile.full_name}`);
        console.log('[v0] User profile:', profile);
      }

      // Test 3: Update profile
      updateResult('Update Profile Data', 'pending', 'Testing profile update...');
      ...
      if (updateError) {
        updateResult('Update Profile Data', 'error', `Error: ${updateError.message}`);
      } else {
        updateResult('Update Profile Data', 'success', 'Profile updated successfully');
        console.log('[v0] Updated profile:', updatedProfile);
      }

      // Test 4: Fetch letters received
      updateResult('Fetch Received Letters', 'pending', 'Fetching inbox letters...');
      ...
      if (lettersError) {
        updateResult('Fetch Received Letters', 'error', `Error: ${lettersError.message}`);
      } else {
        updateResult('Fetch Received Letters', 'success', `Found ${lettersReceived?.length || 0} received letters`);
      ...
      if (sentError) {
        updateResult('Fetch Sent Letters', 'error', `Error: ${sentError.message}`);
      } else {
        updateResult('Fetch Sent Letters', 'success', `Found ${lettersSent?.length || 0} sent letters`);
        console.log('[v0] Sent letters:', lettersSent);
      }

      // Test 6: Fetch user's stamps
      updateResult('Fetch User Stamps', 'pending', 'Fetching stamp collection...');
      ...
      if (stampsError) {
        updateResult('Fetch User Stamps', 'error', `Error: ${stampsError.message}`);
      } else {
        updateResult('Fetch User Stamps', 'success', `Found ${stamps?.length || 0} stamps collected`);
        console.log('[v0] User stamps:', stamps);
      }

      // Test 7: Count statistics
      updateResult('Calculate Stats', 'pending', 'Calculating user statistics...');
      try {
        ...
        if (recError || sentCountError || stampCountError) {
          updateResult('Calculate Stats', 'error', 'Error calculating some statistics');
        } else {
          ...
          console.log('[v0] Stats:', { received: receivedCount, sent: sentCount, stamps: stampCount });
        }
      } catch (e) {
      ...
      updateResult('Fetch All User Profiles', 'pending', 'Fetching all user profiles...');
      ...
      if (allProfilesError) {
        updateResult('Fetch All User Profiles', 'error', `Error: ${allProfilesError.message}`);
      } else {
        updateResult('Fetch All User Profiles', 'success', `Found ${allProfiles?.length || 0} user profiles`);
  ...
  return (
    <div className="p-8 space-y-8">
      ...
      <div className="space-y-2">
        <h1 className="text-3xl font-serif font-bold text-foreground">Database Tests</h1>
        <p className="text-muted-foreground">Verify that all database operations are working correctly</p>
      </div>
      ...
      <div className="space-y-3">
        ...
        {results.map((result) => (
          <div key={result.name} className="postal-card p-4 flex items-start gap-4">
      ...
      {results.length > 0 && (
        <div className="postal-card p-6 bg-muted/30 space-y-2">
          ...
          <p className="text-xs text-muted-foreground">Open your browser&apos;s developer console (F12) to see detailed logs of all database operations. Look for messages starting with &quot;[v0]&quot;</p>
        </div>
      )}
    </div>
  );
